...
export function BrowsePage() {
  ...
  const listQuery = useQuery({
    queryKey: searching
      ? ['search', submitted, page]
      : ['discover', genreId, year, sortBy, page],
    queryFn: () =>
      searching
        ? searchMovies(submitted, page)
        : discoverMovies({ page, genreId, year, sortBy }),
    enabled: hasPublicAuth(),
  })
  ...
  if (!hasPublicAuth()) {
    return (
      <EmptyState title="TMDB key required">
        <p>Add credentials to .env to search and filter movies.</p>
      </EmptyState>
    )
  }
  ...
  return (
    <section className="space-y-6">
      ...
      {listQuery.data ? <MovieGrid movies={listQuery.data.results} /> : null}
